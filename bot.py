...
def cancel_all_open_orders(api_key, api_sec, cano, acnt_prdt, env_str, port_id, token, is_mock):
    ...
    for order in active_orders:
        if not order['broker_order_id'] or not order['branch_no']:
            continue
...
def main_loop():
    ...
    while True:
        try:
            ...
            if not token:
                print("⚠️ Token Issuance Failed.")
                time.sleep(10)
                continue

            active_strat_str = db.get_setting('strategy', 'CORE')
            port_id = active_strat_str
            strat_id = active_strat_str
            
            sys_status = db.get_system_status("KIS", env_str, cano, port_id)
            
            if sys_status["contract_version"] != db.CONTRACT["contract_version"]:
                print("🚨 Version Mismatch. Halting.")
                time.sleep(10)
                continue
            ...
            while True:
                ...
                if check_sys["kill_switch"] or not check_sys["auto_trade"]:
                    db.transition_order_status(oid, 'CLAIMED', 'CANCELED', code="SYS_BLOCKED")
                    break
                ...
                if not is_ok:
                    ...
                    print(f"🛑 [Risk Block] {tk}: {reason}")
                    continue

                if not db.transition_order_status(oid, 'CLAIMED', 'SUBMITTING'):
                    continue
                ...
                if status == "ACKNOWLEDGED":
                    ...
                    db.transition_order_status(oid, 'SUBMITTING', status, code=code)
