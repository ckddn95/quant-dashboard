...
class OrderSpec:
    ...
    portfolio_id: str; strategy_id: str; strategy_version: str; ticker: str; stock_name: str; side: str; order_kind: str
    quantity: int; limit_price: float; intent_created_at: str
...
def run_quant_simulation(target_stocks_df: pd.DataFrame, strat: Strategy, init_cash: float, start_date: datetime.date, end_date: datetime.date, cfg: StrategyConfig, is_weekly_scan: bool = False):
    try:
        ...
        cash, positions, nav_history, pending_orders, cooldown_tracker = float(init_cash), {}, [], [], {}
        trade_log, closed_trades_log = [], []
        ...
        for i, current_date in enumerate(calendar):
            for order in pending_orders:
                ...
                if order['side'] == 'BUY':
                    ...
                    closed_trades_log.append({
                        "종목명": ticker_to_name.get(tk, tk), "진입일": positions[tk]["entry_date"].strftime('%Y-%m-%d'), "청산일": current_date.strftime('%Y-%m-%d'),
            ...
            for tk, pos in list(positions.items()):
                ...
                if is_sell and sell_price > 0 and "종가 추세이탈" not in reason:
                    ...
                    if profit_pct < 0: cooldown_tracker[tk] = current_date
                    closed_trades_log.append({
                        "종목명": ticker_to_name.get(tk, tk), "진입일": pos["entry_date"].strftime('%Y-%m-%d'), "청산일": current_date.strftime('%Y-%m-%d'),
            ...
            for tk, pos in positions.items():
                ...
                except: daily_eval += pos['qty'] * pos['buy_price']
            nav_history.append({"Date": current_date, "NAV": daily_eval})
            
            daily_pnl_pct = (daily_eval / nav_history[-2]["NAV"]) - 1.0 if len(nav_history) >= 2 and nav_history[-2]["NAV"] > 0 else 0.0
            
            # 🛑 [패치] DAILY_APPROX 강제 적용 (시스템 룰)
        ...
        for tk, pos in positions.items():
            ...
            closed_trades_log.append({
                ...
                "진입단가": pos['buy_price'], "청산단가": last_close, "수량": pos['qty'], "손익금": profit_amt, "수익률": f"{profit_pct*100:+.2f}%", "사유": "보유 중 (미청산)"
            })
        ...
        return {
            "status": "success", "final_asset": final_asset, "final_port_ret": (final_asset / init_cash - 1) * 100, "metrics": {"CAGR": cagr, "MDD": mdd},
            "trade_logs": closed_trades_log, "summary_rows": [{"항목": "총 거래일수", "값": f"{len(nav_df)} 일"}, {"항목": "총 매도(청산) 횟수", "값": f"{len(trade_log)} 회"}, {"항목": "승률 (Win Rate)", "값": f"{win_rate:.1f} %"}, {"항목": "가정 수수료율", "값": f"{assumed_cost_pct*100:.2f}% (ALL-IN)"}, {"항목": "데이터 처리 모드", "값": "DAILY_APPROX (일봉 근사, 분봉 미검증)"}]
        }
