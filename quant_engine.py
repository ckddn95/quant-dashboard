...
def evaluate_stock_for_ui(ticker, strat, buy_price=0.0, highest_price=0.0, use_ma200=True, buf_pct=0.015, ts_tgt=0.30, ts_drp=-0.10, sl=-0.15, min_h=5, c_price=0.0):
    try:
        ...
        if buy_price > 0:
            ...
            if ret <= sl: return c_price, "🔴 긴급 손절 매도", 10.0, f"수익률 {ret*100:+.1f}%"
            if highest_price > 0 and (highest_price/buy_price - 1) >= ts_tgt:
                drop_from_peak = (c_price / highest_price) - 1
                if drop_from_peak <= ts_drp: return c_price, "🔵 트레일링 익절", 20.0, f"하락 {drop_from_peak*100:+.1f}%"
            if strat == "Core" and c_price < ma60 * (1 - buf_pct/2): return c_price, "🔴 전량 청산", 30.0, f"60일선 이탈"
            elif strat == "Satellite" and c_price < ma20 * (1 - buf_pct/2): return c_price, "🔴 전량 청산", 30.0, f"20일선 이탈"
...
def run_quant_simulation(sim_stocks, strat, init_cash, start_date, end_date, use_ma200, w_buf, sl, max_alloc_pct, min_h, ts_tgt, ts_drp, b_boost, cd_days):
    ...
    for curr_date in all_trade_dates:
        ...
        for tk, val in sim_data.items():
            ...
            if strat == "Core" and pass_ma200 and (ma20 >= ma60 * (1 + w_buf)) and m60_up: buy_flag = True
            elif strat != "Core" and pass_ma200 and (-0.05 <= (cp/ma20)-1 <= 0.03): buy_flag = True
...
def run_yearly_realistic_backtest(strat, init_cash, year, use_ma200, w_buf, sl, max_alloc_pct, ts_tgt, ts_drp, b_boost, cd_days, min_h):
    ...
    for i, current_date in enumerate(all_trade_dates):
        ...
        for q in buy_queue:
            ...
            alloc_fund = min(cash, total_equity * (current_alloc_pct / 100.0))
        ...
        if current_date.weekday() == 0 or i == 0:
            ...
            for tc, val in data_dict.items():
                if loss_streak.get(tc, 0) >= 2 and tc in last_loss_date and (current_date - last_loss_date[tc]).days < cd_days: continue 
                df = val['df']
                if current_date not in df.index: continue
                ...
                if use_ma200 and c_p < ma200: continue
                if strat == 'Core' and ma20 >= ma60 * (1 + w_buf) and m60_up: weekly_watchlist.append({'tk': tc, 'name': val['name'], 'score': min(85.0 + max(0, ((ma20/ma60)-1)*100), 99.0)})
        ...
        for tc in list(positions.keys()):
            ...
            can_trend_exit = (current_date - pos['buy_date']).days >= min_h
            ...
            if sell_price == 0.0 and can_trend_exit:
                ...
                if strat == "Core" and close_p < ma60 * (1 - w_buf/2): sell_price, sell_reason = close_p, f"🔴 종가 추세이탈"
                elif strat == "Satellite" and close_p < ma20 * (1 - w_buf/2): sell_price, sell_reason = close_p, f"🔴 종가 추세이탈"
        ...
        if len(positions) < dynamic_max_slots:
            for w in weekly_watchlist:
                ...
                buy_signal = False
                if strat == "Core" and pass_ma200 and (ma20 >= ma60 * (1 + w_buf)) and m60_up: buy_signal = True
                elif strat != "Core" and pass_ma200 and (-0.05 <= (c_p / ma20) - 1 <= 0.03): buy_signal = True
                if buy_signal:
