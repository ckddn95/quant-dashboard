...
def run_worker_loop():
    ...
    while True:
        try:
            ...
            logger.error(f"Worker Error: {e}"); time.sleep(5)import time
import logging
import uuid
import datetime
import traceback
import os
import math  # 🚨 패치: Inf, NaN 수학적 검증을 위한 모듈 추가
try: import tomllib as toml
except ImportError: import toml
import database as db
import broker.kis_client as kis
    ...
    while True:
        try:
            for portfolio_id in ["CORE", "SATELLITE"]:
                ...
                # 🚨 패치 3: 워커의 생존을 증명하는 실시간 Heartbeat 기록 (UI 대시보드와 연동)
                try: 
                ...
                # 🚨 패치 2: 시세 데이터의 수학적/논리적 무결성 극단적 엄격 검증 (Inf, NaN, 역전 방어)
                try:
