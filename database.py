...
def get_locked_cash_and_qty(broker, env, account_id, portfolio_id, ticker=None):
    with get_connection() as conn:
        ...
        r1 = conn.execute(f"SELECT SUM((qty - cum_filled_qty) * limit_price * ?) as locked_cash FROM order_intents WHERE broker=? AND environment=? AND account_id=? AND portfolio_id=? AND side='BUY' AND status IN {open_states}", (cost_multiplier, broker, env, account_id, portfolio_id)).fetchone()
...
def get_portfolio_creation_date(broker, env, account_id, portfolio_id):
    with get_connection() as conn:
        try:
            r1 = conn.execute("SELECT MIN(created_at) as d FROM order_intents WHERE broker=? AND environment=? AND account_id=? AND portfolio_id=?", (broker, env, account_id, portfolio_id)).fetchone()
