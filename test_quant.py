...
# CI 테스트 모드 강제 활성화 (데이터베이스 사전 검사 우회)
os.environ["CI_TEST_MODE"] = "true"

class QuantIntegrityTest(unittest.TestCase):
    def setUp(self):
        if os.path.exists("test_quant_system.db"): os.remove("test_quant_system.db")
        db.bootstrap_db()
    ...
    def test_03_strict_real_block(self):
        """REAL 경로 차단 검증"""
        now = datetime.datetime.now(quant.KST).strftime('%Y-%m-%d %H:%M:%S')
        spec = quant.OrderSpec("t3", "t3", "KIS", "REAL", "testfp", "01", "CORE", "CORE", "1.0.0", "1.0.0", "005930", "삼성전자", "BUY", "MARKET", 10, 0, 50000, "KRX", "GTC", "SYSTEM", "SYSTEM", now, 60, "2.2.0", now)
        db.safe_add_order_intent(spec)
        with db.get_connection() as conn:
    ...
    def test_04_version_mismatch(self):
        """버전 불일치 격리 검증"""
        now = datetime.datetime.now(quant.KST).strftime('%Y-%m-%d %H:%M:%S')
        spec = quant.OrderSpec("t4", "t4", "KIS", "MOCK", "testfp", "01", "CORE", "CORE", "0.0.1", "0.0.1", "005930", "삼성전자", "BUY", "MARKET", 10, 0, 50000, "KRX", "GTC", "SYSTEM", "SYSTEM", now, 60, "0.0.1", now)
        db.safe_add_order_intent(spec)
        with db.get_connection() as conn:
    ...
    def test_05_kis_payload_completeness(self):
        ...
        # 매수 주문 생성
        buy_data = kis.execute_kis_order_001x("K", "S", "123", "01", "T", "005930", True, 10, 0, True)
        # kis_client 내부 로직상 매수는 SLL_TYPE "" (데이터 구성 확인은 mok_post 모킹 필요하나 여기선 페이로드 로직 자체 검증)
        
        # 내부 로직 직접 호출하여 매수/매도 SLL_TYPE 분기 확인
    ...
    def test_06_manual_order_atomic_fill(self):
        """수동 주문과 자동 주문의 managed_qty 분리 검증"""
        now = datetime.datetime.now(KST).strftime('%Y-%m-%d %H:%M:%S')
        spec = quant.OrderSpec("t6", "t6", "KIS", "MOCK", "testfp", "01", "CORE", "CORE", "1.0.0", "1.0.0", "005930", "삼성전자", "BUY", "MARKET", 10, 0, 50000, "KRX", "GTC", "UI_MANUAL", "UI_MANUAL", now, 99999, "2.2.0", now)
        db.safe_add_order_intent(spec)
        with db.get_connection() as conn:
    ...
    def test_07_atomic_fencing(self):
        ...
        now = datetime.datetime.now(KST).strftime('%Y-%m-%d %H:%M:%S')
        spec = quant.OrderSpec("t7", "t7", "KIS", "MOCK", "testfp", "01", "CORE", "CORE", "1.0.0", "1.0.0", "005930", "삼성전자", "BUY", "MARKET", 10, 0, 50000, "KRX", "GTC", "SYSTEM", "SYSTEM", now, 99999, "2.2.0", now)
        db.safe_add_order_intent(spec)
        with db.get_connection() as conn:
